...
def print_over():
    ...
    txt1 = over_font.render('GAME OVA', True, (255, 255, 255))
    screen.blit(txt1, (300, 200))
    txt2 = font.render('PRESS SPACE TO RESTART', True, (255, 255, 255))
    screen.blit(txt2, (150, 270))


start_font = pg.font.Font('freesansbold.ttf', 48)
def print_start():
    global SCREEN_WIDTH, SCREEN_HEIGHT
    txt2 = start_font.render('PRESS SPACE TO BEGIN', True, (255, 255, 255))
    screen.blit(txt2, (150, 270))
...
while running:
    if game_state == 'playing':
        ...
        for event in pg.event.get():
            ...
            # Check if keystroke to restart game
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_SPACE:
                    game_state = 'playing'
                    enemys_a_X = [((SCREEN_WIDTH - ENEMY_A_SIZE) / 2) + 40*i for i in range(N_ENEMIES)]
